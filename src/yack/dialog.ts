 ...
 *       endif
 */
export function tryParseChoice(line: string, lines: string[], index: number): ParsedChoice | null {
    if (!line.startsWith("* ")) {
        return null;
    }
...
function tryParseMultiLine(firstLine: string, lines: string[], index: number): ParsedChoice | null {
